...
/** Test/demo verisi bu önekle işaretlenir — hem oluşturma hem temizleme bu değere göre çalışır. */
export const SAMPLE_PRODUCT_PREFIX = '[Örnek] '

export async function fetchSampleProducts(): Promise<Product[]> {
  const { data, error } = await supabase
    .from('products')
    .select('*')
    .like('name', `${SAMPLE_PRODUCT_PREFIX}%`)
    .eq('is_active', true)
  if (error) throw error
  return data as Product[]
}

/** Kullanıcının hızlıca deneyip sonra temizleyebileceği birkaç örnek ürün + hareket ekler — hepsi "[Örnek] " önekiyle işaretli, gerçek verilerle asla karışmaz. */
export async function seedSampleStockData(): Promise<void> {
  const samples: ProductInput[] = [
    {
      name: `${SAMPLE_PRODUCT_PREFIX}Test Ürünü A`,
      unit: 'adet',
    ...
    {
      name: `${SAMPLE_PRODUCT_PREFIX}Test Ürünü B (Flakon)`,
      unit: 'adet',
      critical_stock_threshold: 5,
      category: 'Cilt Bakımı',
      brand_line: 'swiss',
      flakon_per_package: 2,
    },
    {
      name: `${SAMPLE_PRODUCT_PREFIX}Test Ürünü C`,
      unit: 'kutu',
...
/**
 * "[Örnek] " önekli tüm test ürünlerini kaldırır — gerçek "Ürünü kaldır" ile
 * AYNI yumuşak silme (is_active=false) kullanılıyor, kalıcı DELETE değil.
 * Kalıcı silme denendi ama bazı ürünlerin sample_items (numune talepleri) gibi
 * başka tablolardan yabancı anahtar referansı olabiliyor ve bu, silmeyi
 * ürün başına öngörülemez şekilde reddedip yarım kalmış bir temizliğe yol
 * açabiliyordu — yumuşak silme bu riski tamamen ortadan kaldırıyor, gerçek
 * ürünlerdeki "kaldır" davranışıyla da tutarlı.
 */
export async function clearSampleStockData(): Promise<number> {
