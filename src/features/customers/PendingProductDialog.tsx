...
import { useCreatePendingProduct } from './hooks'
...
export function PendingProductDialog({ customerId }: { customerId: string }) {
  const [open, setOpen] = React.useState(false)
  const createMutation = useCreatePendingProduct()
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">
          <Plus className="size-3.5" /> Eksik Ürün Ekle
        </Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Eksik / Teslim Edilmemiş Ürün Ekle</DialogTitle>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Örn. Kongre sonrası tamamlanacak" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
