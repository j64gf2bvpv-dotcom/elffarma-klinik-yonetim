...
// Hatırlatmalar sayfası açılıp görüntülendiğinde o an listede olan hatırlatmaları
// sessizce (toast göstermeden) topluca temizlemek için kullanılır.
export function useDeleteRemindersBulk() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (ids: string[]) => Promise.all(ids.map((id) => deleteReminder(id))),
    onSuccess: (_data, ids) => {
      queryClient.setQueryData<Reminder[]>(['reminders'], (old) => old?.filter((r) => !ids.includes(r.id)))
      queryClient.invalidateQueries({ queryKey: ['reminders'] })
    },
    onError: (error: Error) => toast.error('Hatırlatmalar silinemedi', { description: error.message }),
  })
}
