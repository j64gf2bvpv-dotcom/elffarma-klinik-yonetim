...
export function InvoiceDialog({ payment }: { payment: Payment }) {
  ...
  async function handleViewFile() {
    ...
    setOpening(true)
    try {
      const url = await getInvoiceFileUrl(payment.invoice_file_path)
      // Electron's main process intercepts any window.open() and routes it to
      // shell.openExternal (see setWindowOpenHandler in electron/main.ts).
      window.open(url, '_blank', 'noopener,noreferrer')
    } catch (error) {
      toast.error('Fatura açılamadı', { description: (error as Error).message })
    } finally {
      setOpening(false)
    }
  }
      ...
      <DialogContent>
        ...
        <div className="grid gap-4">
          ...
          {payment.invoice_file_path && (
            <Button type="button" variant="outline" size="sm" onClick={handleViewFile} disabled={opening}>
