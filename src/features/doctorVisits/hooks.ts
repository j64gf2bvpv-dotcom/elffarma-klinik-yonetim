...
import { createVisit, deleteVisit, fetchVisitsByCustomer, fetchVisitsInRange, updateVisit, type DoctorVisitInput } from './api'
...
export function useVisitsByCustomer(customerId: string | undefined) {
  return useQuery({
    queryKey: ['doctor_visits', 'by_customer', customerId],
    queryFn: () => fetchVisitsByCustomer(customerId as string),
    enabled: !!customerId,
  })
}
