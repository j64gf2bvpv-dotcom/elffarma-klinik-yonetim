...
interface LedgerRow {
  date: string
  type: 'Tahsilat' | 'Satış' | 'İade' | 'Fatura' | 'Devir Bakiyesi'
  description: string
  debit: number
  credit: number
}
...
export function CariHesapPage() {
  ...
  const rows = React.useMemo<LedgerRow[]>(() => {
    ...
    if (customer && customer.total_debt != null && Number(customer.total_debt) !== 0) {
      list.push({
        date: customer.created_at,
        type: 'Devir Bakiyesi',
        description: 'Ticari Alacaklar (devir bakiyesi)',
        debit: Number(customer.total_debt),
        credit: 0,
      })
    }
  ...
  }, [customer, payments, sales, invoices])
  ...
  return (
    <div>
                ...
                {rowsWithBalance.map((r, i) => (
                  <TableRow key={i}>
                    ...
                    <TableCell>
                      <Badge
                        variant={
                          r.type === 'Satış' || r.type === 'Fatura' || r.type === 'Devir Bakiyesi'
                            ? 'destructive'
                            : r.type === 'Tahsilat'
                              ? 'success'
                              : 'outline'
      ...
      <p className="text-muted-foreground mt-4 text-xs">
        Not: Bu ekstre satış/iade, fatura ve tahsilat hareketleri ile{' '}
        <Link to={`/musteriler/${id}`} className="text-primary hover:underline">
          doktor profilindeki
        </Link>{' '}
        elle girilen "Ticari Alacaklar" tutarının devir bakiyesi olarak eklenmesinden hesaplanır.
      </p>
    </div>
  )
}
